...
Select COUNT(DISTINCT Category) as ProCategories from  Products
...
select ProductID,SUM(SaleAmount) SumProduct from Sales
 where ProductID=7
 group by ProductID
...
select DepartmentName, Count(EmployeeID) as Employee_Dep from Employees
...
select CustomerID, Sum(SaleAmount) as Sum_Sale from Sales
group by CustomerID
...
--DeptID is not given in the table, so we must use DeptName instead
...
select ProductID, Sum(SaleAmount) as Sum_Sales, AVG(SaleAmount) as Aver_Sale from Sales
group by ProductID
...
select DepartmentName, Count(EmployeeID) As Dep_HR_Emp from Employees
...
--DeptID is not given in the table, so we must use DeptName instead
Select DepartmentName, AVG(salary) as Aver_Sal from Employees
group by DepartmentName
...
--DeptID is not given in the table, so we must use DeptName instead
Select DepartmentName, AVG(salary) as Aver_Sal,Count(EmployeeID) as Number_Empl from Employees
group by DepartmentName

--16 th question
--Write a query that uses HAVING to filter product categories with an average price greater than 400.
Select Category,AVG(Price) as Aver_Price from Products
group by Category
...
Select Year(SaleDate) as Year_Sale,Sum(SaleAmount) as Sum_Sale from Sales
...
select CustomerID,Count(*) as Num_Cust from Orders
group by CustomerID
Having Count(*)>=3
...
--DeptID is not given in the table, so we must use DeptName instead
Select DepartmentName,Sum(Salary) as Tot_Salary from Employees
group by DepartmentName
...
Select Category,AVG(Price) as Aver_Price from Products
group by Category having AVG(Price)>200
...
Select CustomerID,Sum(SaleAmount) as Tot_sale from Sales
...
Select DepartmentName,Sum(Salary) as Tot_Sal,AVG(Salary) as Aver_Sal from Employees
...
select CustomerID,Max(SaleAmount) as Max_Amount, Min(SaleAmount) as Min_Amount from Sales
group by CustomerID Having Max(SaleAmount)>=50
...
Select  MONTH(SaleDate) as Month_Sale,Count(distinct ProductID) as Dis_Prod from Sales
group by MONTH(SaleDate) having Count(distinct ProductID)>8
--25 th question
--Write a query to find the MIN and MAX order quantity per Year. From orders table. (Do some research)

Select Year(OrderDate) as Years,Min(Quantity) as Min_Quan, Max(Quantity) as Max_Quan from Orders
