...
select distinct CONCAT(FirstName, + ' ' +LastName) as CustName,Country from Customers
...
select *, Case
when Price> 1000 then 'High' else 'Low'end as Pric_of_Products
from Products
--7 th question
---Use IIF to create a column that shows 'Yes' if Stock > 100, and 'No' otherwise (Products_Discounted table, StockQuantity column).
select * from Products_Discounted

select *,IIF(StockQuantity>100,'Yes', 'No') as Confirmation from Products_Discounted
---Medium-Level Tasks
---8 th question
...
select *,IIF(Price>1000, 'Expensive', 'Affordable') as Price_Types from Products
...
when EmployeeID = 5 then Salary*1.1 else salary end as Updated_Salary
from Employees
...
when SaleAmount<=500 and SaleAmount>=200 then 'Mid Tier' else 'Low Tier' end as Sale_Types
from Sales
...
when Quantity>1 and Quantity<=3 then 5  else 7  end as discount_percentage
from Orders
