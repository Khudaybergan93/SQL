...
use homework_10
--1 st task, easy-level tasks
--Write a query to perform an INNER JOIN between Orders and Customers 
--using AND in the ON clause to filter orders placed after 2022.
--first of All we must create both Orders and Customers Tables according to the question
CREATE TABLE Orders (
...
--2 nd task, easy-level tasks
--Write a query to join Employees and Departments using OR in the ON clause to show employees
--in either the 'Sales' or 'Marketing' department.
--first of All we must create both Employees and Departments Tables according to the question
...
--3 rd Task, Easy-Level Tasks
-- Write a query to demonstrate a CROSS APPLY between Departments and a derived table
--that shows their Employees, top-performing employee (e.g., top 1 Employee who gets the most salary).
select e.Name, Max(Salary) as top_salary from Departments d
CROSS APPLY
(select Top 1
Name, Salary from Employees
where DepartmentID=d.DepartmentID) e
group by e.Name 

--4 th Task, Easy-Level Tasks
-- Write a query to join Customers and Orders using AND in the ON clause to filter customers 
--who have placed orders in 2023 and who lives in the USA.
--the question itself says " who have placed orders" which clearly indicates at least 2 or moore orders (not one)
select c.CustomerID, c.FirstName, c.LastName,o.Quantity,c.Country
from Customers c Join Orders o on c.CustomerID=o.CustomerID 
And '2023-01-01'<=o.OrderDate and o.OrderDate<='2023-12-31'and o.Quantity>=2 And c.Country='USA'


--5 th Task, Easy-Level Tasks
-- Write a query to join a derived table (SELECT CustomerID, COUNT(*) FROM Orders GROUP BY CustomerID)
--with the Customers table to show the number of orders per customer.
--the derived table for Customers table is Orders table as we have seen in the previous tasks and it must in subquery for this task
select c.CustomerID, Count_Num, FirstName, LastName from Customers c 
Cross Apply 
(SELECT CustomerID, COUNT(*) as Count_Num
FROM Orders where CustomerID=c.CustomerID
GROUP BY CustomerID) o


--6 th Task, Easy-Level Tasks
--Write a query to join Products and Suppliers using OR in the ON clause 
--to show products supplied by either 'Gadget Supplies' or 'Clothing Mart'.
--first of All we must create both Products and Suppliers Tables according to the question
...
--we can only join these tables through SupplierID which exists in both tables
select p.ProductName from Products p
Inner Join Suppliers s on p.SupplierID=s.SupplierID
AND (s.SupplierName='Gadget Supplies' or s.SupplierName='Clothing Mart')

--7 th Task, Easy-Level Tasks
--Write a query to demonstrate the use of OUTER APPLY between Customers 
--and a derived table that returns each Customers''s most recent order.
...
--8 th Task, Medium-Level Tasks
-- Write a query that uses the AND logical operator in the ON clause to join Orders and Customers, 
--and filter customers who placed an order with a total amount greater than 500.
Select o.CustomerID, Count(distinct o.OrderID) as Dist_Ord, 
c.FirstName, c.LastName,o.TotalAmount, o.OrderID from Orders o
Join Customers c on o.CustomerID=c.CustomerID
And o.TotalAmount>500
group by o.CustomerID,c.FirstName, c.LastName,o.TotalAmount,o.OrderID

--9 th Task, Medium-Level Tasks
--Write a query that uses the OR logical operator in the ON clause to join Products
--and Sales to filter products that were either sold in 2022 or the SaleAmount is more than 400.
--We already have Products table and we must create sales table to continue the process
CREATE TABLE Sales (
...
--10 th Task, Medium-Level Tasks
-- Write a query to join a derived table that calculates the total sales (SELECT ProductID, SUM(Amount)
--FROM Sales GROUP BY ProductID) with the Products table to show total sales for each product.
select s.ProductID, SUM(s.SaleAmount) as Sum_Prod from Sales s
Cross Apply 
(Select * from Products
where ProductID=s.ProductID) p
group by s.ProductID

--11 th Task, Medium-Level Tasks
--Write a query to join Employees and Departments using AND in the ON clause to filter employees
...
--In the provided table 'HR' is represented as 'Human Resources' with full identification for clarification

--12 th task, Medium-Level tasks
--Write a query to use OUTER APPLY to return all customers along with their most recent orders,
...
(select Top 1
* from Orders 
where CustomerID=c.CustomerID) o
order by o.OrderDate desc

--13 th task, Medium-Level Tasks
--Write a query to join Products and Sales using AND in the ON clause to filter products 
--that were sold in 2023 and StockQuantity is more than 50.
Select * from Products
select * from Sales
...
--15 th task, Hard-Level Tasks
--Write a query to demonstrate the use of the AND logical operator in the ON clause between Orders and Customers,
...
c.Country,c.Address from Orders o
Join Customers c on o.CustomerID=c.CustomerID
And c.Country='USA' And c.Address like ('[0-9][0-9][0-9][0-9]%')

--16 th task, Hard-Level Tasks
--Write a query to demonstrate the use of OR in the ON clause when joining Products and Sales 
--to show products that are either part of the 'Electronics' category or Sale amount is greater than 350.
Select p.ProductName,p.Category,
s.SaleAmount from Products p
join Sales s on p.ProductID=s.ProductID
And (p.Category='Electronics' Or s.SaleAmount>350)

--17 th task, Hard-Level Tasks
--Write a query to join a derived table that returns a count of products per category 
--(SELECT CategoryID, COUNT(*) FROM Products GROUP BY CategoryID) with the Categories table to show the count of products in each category.
--we have products table and we must create Categories table
...
SELECT p.Category, COUNT(ProductName) as Pro_Count FROM Products p
Cross Apply(
select * from Categories
where CategoryName=p.Category) c
GROUP BY p.Category
--Actually, to count number of products by each category we must use 'SELECT p.Category, COUNT(ProductName) as Pro_Count FROM Products'
--not category ID

--18 th task, Hard-Level Tasks
--Write a query to join Orders and Customers using AND in the ON clause to show orders 
--where the customer is from 'Los Angeles' and the order amount is greater than 300.
select o.OrderID,c.FirstName,c.LastName,c.City from Orders o
join Customers c on o.CustomerID=c.CustomerID
And c.City='Los Angeles' and o.TotalAmount>300

--19 th task, Hard-Level Tasks
--Write a query to join Employees and Departments using a complex OR condition in the ON clause 
--to show employees who are in the 'HR' or 'Finance' department, or have at least 4 wowels in their name.
Select e.Name,d.DepartmentName from Employees e
join Departments d on e.DepartmentID=d.DepartmentID
And (d.DepartmentName='Human Resources' Or d.DepartmentName='Finance' Or e.Name like '%[aeiou]%[aeiou]%[aeiou]%[aeiou]%')
---in the given table 'HR' department was added as Human Resources

--20 th task, Hard-Level Tasks
--Write a query to join Sales and Products using AND in the ON clause to filter products 
--that have both a sales quantity greater than 100 and a price above 500.
Select * from Sales
select * from Products

Select p.ProductID,p.ProductName,
p.StockQuantity,p.Price from Sales s
join Products p on s.ProductID=p.ProductID
And p.StockQuantity>100 And p.Price>500

--21 st task, Hard-Level Tasks
--Write a query to join Employees and Departments using OR in the ON clause to 
--show employees in either the 'Sales' or 'Marketing' department, and with a salary greater than 60000.
Select e.Name,d.DepartmentName,e.Salary from Employees e
join Departments d on e.DepartmentID=d.DepartmentID
And (d.DepartmentName='Sales' or  d.DepartmentName='Marketing') and e.Salary>60000 
