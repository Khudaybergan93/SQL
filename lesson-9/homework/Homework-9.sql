...
--We already have both Employees and Depatments tables 
...
select e.Name, ISNull(d.Name, 'Unknown') as ManagaerName from Employees e
...
Select e.DepartmentID, d.DepartmentName, count(e.EmployeeID) as Emp_Count 
from Employees e
Inner Join Departments d on e.DepartmentID=d.DepartmentID
...
select e.EmployeeID, e.Name as EmployeeName, d.EmployeeID as ManagerID,
d.Name as ManagerName from Employees e                              
join Employees d on e.ManagerID=d.EmployeeID 
order by d.EmployeeID, e.EmployeeID
...
select e.Name from Employees e
Inner Join Departments d on e.DepartmentID=d.DepartmentID
where d.DepartmentName='Sales' and Salary>5000
...
select p.ProductName from Products p
FULL OUTER JOIN Categories c on p.Category=c.CategoryName
where p.Category is null

--23 rd Task, Hard-Level Tasks
select e.EmployeeID,e.Name as Emp_Name,
m.EmployeeID as ManagerID, m.Name as Managername from 
Employees e
join Employees m on e.ManagerID=m.EmployeeID
where e.Salary>5000 
order by m.EmployeeID, e.ManagerID
...
where p.Category='Electronics' or p.Category='Furniture'
