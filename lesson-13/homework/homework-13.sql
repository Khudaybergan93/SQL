...
Select Concat(EMPLOYEE_ID, '-', FIRST_NAME, ' ',LAST_NAME) as EMP_id_Name from Employees
...
Select E2.EMPLOYEE_ID,E1.FIRST_NAME,E1.LAST_NAME, E2.SALARY from Employees as E1 join Employees as E2 
...
when Max3>=Max2 and Max3>=Max1 then Max3 else 0 end as Biggest_Results
from TestMax
...
--Display their Employee ID, First Name, Last Name, Hire Date, and the Years of Service (calculated 
--as the number of years between the current date and the hire date, rounded to two decimal places).(Employees)
Select * from Employees

SELECT 
    EMPLOYEE_ID,FIRST_NAME,LAST_NAME,HIRE_DATE,
    ROUND(DATEDIFF(Year, Getdate(), HIRE_DATE) / 365.25, 2) AS YearsOfService
FROM 
    Employees
WHERE 
    DATEDIFF(Year, Getdate(), HIRE_DATE) / 365.25 > 10
    AND DATEDIFF(Year, Getdate(), HIRE_DATE) / 365.25 < 15;
--10 th question
--Find the employees who have a salary greater than the average salary of their respective department.(Employees)
Select * from Employees

Select * from Employees as Outs where SALARY>(Select AVG(SALARY) from Employees as Ins where Outs.DEPARTMENT_ID=Ins.DEPARTMENT_ID)
...
WITH input_string AS (
    SELECT 'tf56sd#%OqH' AS input
),
split_chars AS (
    SELECT 
        SUBSTRING(input, number, 1) AS ch
    FROM input_string
    JOIN (
    ...
    ) AS numbers ON number <= LEN(input)
),
categorized AS (
    SELECT
        ch,
        CASE WHEN ASCII(ch) BETWEEN 65 AND 90 THEN ch ELSE NULL END AS upper,
        CASE WHEN ASCII(ch) BETWEEN 97 AND 122 THEN ch ELSE NULL END AS lower,
        CASE WHEN ASCII(ch) BETWEEN 48 AND 57 THEN ch ELSE NULL END AS number,
        CASE 
            WHEN ASCII(ch) NOT BETWEEN 48 AND 57 AND
                 ASCII(ch) NOT BETWEEN 65 AND 90 AND
                 ASCII(ch) NOT BETWEEN 97 AND 122 THEN ch 
            ELSE NULL 
        END AS other
    FROM split_chars
)
SELECT
    STRING_AGG(upper, '') AS uppercase_letters,
    STRING_AGG(lower, '') AS lowercase_letters,
    STRING_AGG(number, '') AS numbers,
    STRING_AGG(other, '') AS other_characters
FROM categorized;
...
FROM Students;
--13 th question
--For every customer that had a delivery to California, provide a result set of the 
--customer orders that were delivered to Texas. (Orders Table)
...
--14 th question
--Write an SQL query to transform a table where each product has a total quantity into a new table 
...
Select Concat(SequenceNumber, ' ', String) as Concatination from DMLTable
--16 th question
...
SELECT 
    ...
    LAST_NAME,
    HIRE_DATE,
    FLOOR(DATEDIFF(Year, GETDATE(), HIRE_DATE)) AS YearsWorked,
    CASE 
...
SELECT 
    JobDescription,
    MIN(CASE WHEN LeastRank = 1 THEN SpacemanID END) AS LeastExperiencedID,
    MIN(CASE WHEN MostRank = 1 THEN SpacemanID END) AS MostExperiencedID
...
with cte as (
select *, ROW_NUMBER() over (order by (select Null)) as ROW_NUM from Students
)
select *, cte.Grade+Isnull(prev.Grade, 0) from cte left join cte as Prev on cte.ROW_NUM=Prev.ROW_NUM+1

--22 nd question
--Given the following hierarchical table, write an SQL statement that determines 
--the level of depth each employee has from the president. (Employee table)
...
WITH Hierarchy AS (
    -- Anchor: Start from the President (level 0)
    SELECT 
        EmployeeID,
        ManagerID,
        JobTitle,
...
-- Cursor to loop over all equations
...
BEGIN
    ...
    SET @SQL = N'SELECT @ResultOut = ' + @Equation;
...
---Birthday is not provided so we will have to add it

Alter table Students
add Birtday date

update Students
set Birtday='2001-01-02'
where StudentID=1

update Students
set Birtday='2004-01-02'
where StudentID=2
...
update Students
set Birtday='2002-01-02'
where StudentID=4

update Students
set Birtday='1999-07-02'
where StudentID=5

update Students
set Birtday='1998-09-23'
where StudentID=6
