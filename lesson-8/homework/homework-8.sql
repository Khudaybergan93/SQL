...
select Category,Count(*) as Tot_Products from Products
group by Category

--2 nd question
--Using Products table, get the average price of products in the 'Electronics' category.
select Category,Avg(Price) as Tot_Products from Products
...
Select *,IIF(StockQuantity<30,'Low Stock', 'Sufficient') as Number_Stock from Products

--8 th question
--Using Customers table, find the total number of customers in each country.
Select Country,Count(*) as Country_Cust from Customers
group by Country
...
select Max(Quantity) as Max_Quant, Min(Quantity) as Min_Quant from Orders
...
---we have already created Orders table, so we must create only Invoices table
...
--we have already created Products table, so we must create only Products_Discounted table
...
Select Year(OrderDate) as Year,Count(OrderID) as Aver_Order from Orders
...
when Price>=100 and Price<=500 then 'Mid' else 'High' end as Price_Types
from Products
...
select ProductID, Sum(SaleAmount) as Sale_Product from Sales
group by ProductID
--17 th question
...
Select P.ProductName, Count(S.SaleID) as Times from Products as P Join Sales as S on P.ProductID=S.ProductID
group by P.ProductName

---22 nd question
--Using Orders table, find top 5 products (by ProductID) with the highest order quantities.
select top 5 ProductID, Max(Quantity) as Max_Quant from Orders
group by ProductID order by Max_Quant desc